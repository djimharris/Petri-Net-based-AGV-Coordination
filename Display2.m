...
% DISPLAY2 MATLAB code for Display2.fig
%      DISPLAY2, by itself, creates a new DISPLAY2 or raises the existing
%      singleton*.
%
%      H = DISPLAY2 returns the handle to a new DISPLAY2 or the handle to
%      the existing singleton*.
%
%      DISPLAY2('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in DISPLAY2.M with the given input arguments.
%
%      DISPLAY2('Property','Value',...) creates a new DISPLAY2 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Display2_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Display2_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Display2

% Last Modified by GUIDE v2.5 27-Apr-2016 20:09:50

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Display2_OpeningFcn, ...
                   'gui_OutputFcn',  @Display2_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Display2 is made visible.
function Display2_OpeningFcn(hObject, eventdata, handles, varargin)
...
set(handles.r41,'Value',0);
set(handles.r42,'Value',0);
set(handles.r43,'Value',0);

set(handles.r51,'Value',0);
set(handles.r52,'Value',0);
set(handles.r53,'Value',0);
...
handles.taskArray = zeros(6,3);

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes Display2 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = Display2_OutputFcn(hObject, eventdata, handles) 
...
while(get(handles.start,'Value'))
    ...
    handles.taskArray(5,1)=get(handles.r41,'Value');
    handles.taskArray(5,2)=get(handles.r42,'Value');
    handles.taskArray(5,3)=get(handles.r43,'Value');

    handles.taskArray(6,1)=get(handles.r51,'Value');
    handles.taskArray(6,2)=get(handles.r52,'Value');
    handles.taskArray(6,3)=get(handles.r53,'Value');
